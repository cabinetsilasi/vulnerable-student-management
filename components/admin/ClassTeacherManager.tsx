...
export function ClassTeacherManager({
  ...
  const handleOpenEditTeacher = (teacher: TeacherRow) => {
  ...
  const handleSaveEditTeacher = async (e: React.FormEvent) => {
    ...
    try {
      if (onEditTeacher) {
        await onEditTeacher(editingTeacherId, editTeacherName.trim(), editTeacherEmail.trim(), editTeacherPhone.trim())
      }
      setShowEditTeacherModal(false)
    } catch (err: any) {
      alert("❌ Eroare la salvarea dirigintelui: " + (err?.message || "Operațiunea a eșuat."))
    }
  }
  ...
  return (
    <div className="space-y-6">
                      ...
                      <tr key={cls.id} className="hover:bg-slate-50/80 transition-colors">
                        ...
                        <td className="p-4 text-slate-500 text-xs">
                          <div className="flex items-center gap-2 group">
                            ...
                            <button onClick={() => handleOpenEditTeacher(assign.teacher)} className="opacity-0 group-hover:opacity-100 p-1 text-slate-400 hover:text-teal-600 transition-all" title="Editează date diriginte (Email/Telefon)">
      ...
      {/* Modal: Editează Diriginte */}
      {showEditTeacherModal && (
        <div className="fixed inset-0 z-50 bg-slate-950/40 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-xl space-y-4">
            <div className="flex items-center gap-2 text-teal-700 font-bold border-b border-slate-100 pb-3">
              <Edit2 className="w-5 h-5 text-teal-600" />
              <h3 className="text-lg font-bold text-slate-900">Editează Date Diriginte</h3>
            </div>
            ...
            <form onSubmit={handleSaveEditTeacher} className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-teal-500/20 focus:outline-none text-slate-900"
                />
              </div>
